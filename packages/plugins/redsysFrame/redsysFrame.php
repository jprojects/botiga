 ...
 * @since		1.5
 */
class plgBotigaRedsysFrame extends JPlugin
{
	var $gateway = 'RedsysFrame';

	public function __construct(&$subject, $config = array())
	{
		$lang = JFactory::getLanguage();
		$lang->load('plg_botiga_redsysFrame', JPATH_ADMINISTRATOR);
	...
	public function onPaymentNew($paymentmethod, $total, $idComanda)
	{
		...
		$this->params->get('sandbox') == 0 ? $url = 'https://sis-t.redsys.es:25443/sis/NC/sandbox/redsysV2.js' : $url = 'https://sis.redsys.es/sis/NC/redsysV2.js';
		...
		if($lang == 'es-ES') { $idioma = 1; $language = 'ES'; }
		if($lang == 'en-GB') { $idioma = 2; $language = 'EN'; }
		if($lang == 'ca-ES') { $idioma = 3; $language = 'CA'; }
		if($lang == 'fr-FR') { $idioma = 4; $language = 'FR'; }
		if($lang == 'de-DE') { $idioma = 5; $language = 'DE'; }
		if($lang == 'it-IT') { $idioma = 7; $language = 'IT'; }
		if($lang == 'pt-PT') { $idioma = 9; $language = 'PT'; }
		...
		$data = (object)array(
			'url'			=> $url,
			'amount'		=> $amount,
			'fuc'		    => $this->params->get('code',''),
			'terminal'		=> $this->params->get('terminal',''),
			'merchantOrder'	=> $order,
			'logo'          => $logo,
			'language'      => $language,
